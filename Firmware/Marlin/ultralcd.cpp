...
#ifdef TEST
static void heating_for_leveling();
#endif
...
static void filament_load()
{
    ...
    if (degHotend(active_extruder) >= FILAMENT_LOAD_TEMP) target_temp = degHotend(active_extruder) + 10;
    else target_temp = FILAMENT_LOAD_TEMP;

    setTargetHotend(target_temp, active_extruder);

    // while(degHotend(active_extruder) < FILAMENT_LOAD_TEMP){
    while(degHotend(active_extruder) < target_temp){

        lcd_implementation_draw_string(0, "  Heating nozzle...");
    
        sprintf(buffer,"        %d", (int)degHotend(active_extruder));
...
static void filament_unload()
{
    ...
    while(degHotend(active_extruder) < FILAMENT_LOAD_TEMP){
        ...
        sprintf(buffer,"      %d / %d", (int)degHotend(active_extruder), FILAMENT_LOAD_TEMP);
        lcd_implementation_draw_string(1, buffer);
        lcd_implementation_draw_string(3, "   Click to Cancel");
...
static void heating_for_leveling(){
    ...
    if (degHotend(active_extruder) >= LEVELING_TEMP) target_temp = degHotend(active_extruder) + 10;
    else target_temp = LEVELING_TEMP;
    ...
    if (degHotend(active_extruder) >= LEVELING_TEMP) {
        if (onetime_flag) {
            // forward_feeding();
            enquecommand_P(PSTR("M106"));
            enquecommand_P(PSTR("G1 E-40 F500"));
